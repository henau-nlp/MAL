...
class WordEmbeddingLayer(object):
    ...
    def load_embeddings_from_glove_file(self,filename,filter,dim=None):
        ...
        with open(filename, 'r', encoding="UTF-8") as gfile:
            for line in gfile:
                parts = line.split()
                # i = 0
                word = parts[0]
                ...
                if (len(filter) == 0) or (word in filter):
                    vector = [float(p) for p in parts[1:]]
...
if __name__ == '__main__':
    ...
    wem.load_embeddings_from_word2vec_file("../embedding/sgns.weibo.bigram-char",filter=[])
    wem.save_embedding("../embedding/embeddings.pkl")
