...
class Preprocessor(DataProcessor):
    ...
    def load_dataset_init(self, corpus_dir, name: str, entity_type: int, choose_fraction: float, max_seq_len=64,
        ...
        train_data = self._read_text(train_path)
        # labels, texts = train_data['labels'], train_data['words']

        test_data = self._read_text(test_path)
        # eval_labels, eval_texts = test_data['labels'], test_data['words']
        annotated_idx = np.random.choice(np.arange(len(train_data)), size=int(choose_fraction * len(train_data)), replace=False)
        un_annotated_idx = set(range(len(train_data))) - set(annotated_idx)

        datapool = DataPool(annotated_texts=[train_data[idx]['words'] for idx in annotated_idx],
                            annotated_labels=[train_data[idx]['labels'] for idx in annotated_idx],
                            unannotated_texts=[train_data[idx]['words'] for idx in un_annotated_idx],
                            unannotated_labels=[train_data[idx]['labels'] for idx in un_annotated_idx])

        # append statistics report to file.
        return datapool, test_data
